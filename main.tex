...
As is customary in the traditional offline-online paradigm, we assume access to an \textit{a priori} set of \textit{training data} comprising solution snapshots of the full-order model at various spatial, temporal, and parametric instances. %here we denote these training indices by the set $\pstIndicesTraining$ with $\Card{\pstIndicesTraining} = \NTrain$. 
...
\end{enumerate}
We present results for same 150 neural networks presented above. Figure~\ref{fig:burg_rom_results} presents the MSE (Fig.~\ref{fig:burg_rom_results1}-\ref{fig:burg_rom_results3}) and %$\ell^1$ (Fig.~\ref{fig:burg_rom_results4}-\ref{fig:burg_rom_results6}), 
$\ell^{\infty}$ (Fig.~\ref{fig:burg_rom_results7}-\ref{fig:burg_rom_results9}) errors for the various approximate solutions. %Figures~\ref{fig:burg_rom_results1}-\ref{fig:burg_rom_results3} depicts the MSE for the \PSTLSROM, \PSTLONEROM, and the \MLROM\ solutions, respectively. Similarly, Figures~\ref{fig:burg_rom_results1}-\ref{fig:burg_rom_results3} depicts the MSE 
