...
class NLSVD(nn.Module):
    def __init__(self,nchan,nx,num_latent_states,snapshots):
        ...
        input_dim = np.array([nx,nx])
        output_dim = np.array([nx,nx])

        for i in range(0,self.nlayers):
          forward_list.append(nn.Linear(input_dim[i], output_dim[i], (kernel_size,1), padding=(padding_size,0),stride=(1,1),padding_mode='replicate'))
          backward_list.append(nn.Conv2d(output_dim[-i-1], input_dim[-i], (kernel_size,1), padding=(padding_size,0),stride=(1,1),padding_mode='replicate'))
