...
class ConvAutoencoder1d(nn.Module):
    def __init__(self,nchan,nx,num_latent_states):
        ...
        self.t_conv5b = nn.Conv2d(nchan, nchan, (kernel_size,1), padding=(padding_size,0),stride=(1,1),padding_mode='replicate')
    ...
    def decoderb(self, x):
        ...
        x = self.unpool1(x)
        x = self.activation(self.t_conv1b(x))
        x = self.unpool2(x)
        x = self.activation(self.t_conv2b(x))
        x = self.unpool3(x)
        x = self.activation(self.t_conv3b(x))
        x = self.unpool4(x)
        x = self.activation(self.t_conv4b(x))
        x = self.t_conv5b(x)
        return x
    ...
    def decoder(self, x):
        ...
        x = self.activation(self.t_conv4(x))
        x = self.t_conv5(x)
        return x
